...
export async function POST(request: NextRequest) {
  try {
    ...
    const availableCollections = collections.collections?.map((c: any) => c.name) || [];
    let collectionToUse = "";
    ...
    if (!collectionToUse) {
      return NextResponse.json({ 
        success: false, 
        error: "No suitable collection found in Qdrant database. Expected: argo_papers, vector_db, or argo_floats",
